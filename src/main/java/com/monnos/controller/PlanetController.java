...
public class PlanetController {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<Planet>> fetchAllPlanets(){
        return new ResponseEntity<>( planetService.getAllPlanets(), HttpStatus.OK);
    }

    @GetMapping("/idSwapi/{id}")
    public ResponseEntity<?> findOneFromSwapi(@PathVariable("id") String id){
        return new ResponseEntity<>(
                planetService.findOneSwapiPlanet(id), HttpStatus.OK);
    }
    @GetMapping("/allSwapi")
    public ResponseEntity<List<SwapiPlanet>> fetchAllSwapiPlanets(){
        return new ResponseEntity<>(
                planetService.getAllSwapiPlanets(), HttpStatus.OK);
    }

    @GetMapping("/byName/{name}")
    public ResponseEntity<Optional<Planet>> findByName(@PathVariable("name") String name) {
